...
type Config struct {
	...
	AdminGroupID uuid.UUID `validate:"required,uuid"` // Admin group ID
}
...
func LoadConfig(v *validator.Validate) (*Config, error) {
	...
	adminGroupIDStr := getEnv("ADMIN_GROUP_ID", "")
	adminGroupID, err := uuid.Parse(adminGroupIDStr)
	if err != nil {
		return nil, errors.New("invalid ADMIN_GROUP_ID: " + err.Error())
	}
