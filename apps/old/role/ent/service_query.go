...
type ServiceQuery struct {
	...
	withClientAccesses *ClientAccessQuery
...
func (sq *ServiceQuery) QueryGroups() *GroupQuery {
...
// QueryClientAccesses chains the current query on the "client_accesses" edge.
func (sq *ServiceQuery) QueryClientAccesses() *ClientAccessQuery {
	query := (&ClientAccessClient{config: sq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := sq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (sq *ServiceQuery) Clone() *ServiceQuery {
	...
	return &ServiceQuery{
		...
		withClientAccesses: sq.withClientAccesses.Clone(),
...
func (sq *ServiceQuery) WithGroups(opts ...func(*GroupQuery)) *ServiceQuery {
	query := (&GroupClient{config: sq.config}).Query()
...
// WithClientAccesses tells the query-builder to eager-load the nodes that are connected to
// the "client_accesses" edge. The optional arguments are used to configure the query builder of the edge.
func (sq *ServiceQuery) WithClientAccesses(opts ...func(*ClientAccessQuery)) *ServiceQuery {
	query := (&ClientAccessClient{config: sq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	sq.withClientAccesses = query
	return sq
}
...
func (sq *ServiceQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Service, error) {
	...
	if query := sq.withGroups; query != nil {
		if err := sq.loadGroups(ctx, query, nodes,
			func(n *Service) { n.Edges.Groups = []*Group{} },
			func(n *Service, e *Group) { n.Edges.Groups = append(n.Edges.Groups, e) }); err != nil {
			return nil, err
		}
	}
	if query := sq.withClientAccesses; query != nil {
		if err := sq.loadClientAccesses(ctx, query, nodes,
			func(n *Service) { n.Edges.ClientAccesses = []*ClientAccess{} },
			func(n *Service, e *ClientAccess) { n.Edges.ClientAccesses = append(n.Edges.ClientAccesses, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *ServiceQuery) loadGroups(ctx context.Context, query *GroupQuery, nodes []*Service, init func(*Service), assign func(*Service, *Group)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(group.FieldServiceID)
	}
	query.Where(predicate.Group(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(service.GroupsColumn), fks...))
	}))
...
func (sq *ServiceQuery) loadClientAccesses(ctx context.Context, query *ClientAccessQuery, nodes []*Service, init func(*Service), assign func(*Service, *ClientAccess)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Service)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(clientaccess.FieldServiceID)
	}
	query.Where(predicate.ClientAccess(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(service.ClientAccessesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
