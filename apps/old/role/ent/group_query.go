...
func (gq *GroupQuery) QueryService() *ServiceQuery {
	query := (&ServiceClient{config: gq.config}).Query()
...
func (gq *GroupQuery) QueryGroupUsers() *GroupUserQuery {
	query := (&GroupUserClient{config: gq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := gq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (gq *GroupQuery) Clone() *GroupQuery {
	...
	return &GroupQuery{
		...
		withService:    gq.withService.Clone(),
...
func (gq *GroupQuery) WithService(opts ...func(*ServiceQuery)) *GroupQuery {
	query := (&ServiceClient{config: gq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	gq.withService = query
	return gq
}
...
func (gq *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	if query := gq.withService; query != nil {
		if err := gq.loadService(ctx, query, nodes, nil,
			func(n *Group, e *Service) { n.Edges.Service = e }); err != nil {
			return nil, err
		}
	}
	if query := gq.withGroupUsers; query != nil {
		if err := gq.loadGroupUsers(ctx, query, nodes,
			func(n *Group) { n.Edges.GroupUsers = []*GroupUser{} },
			func(n *Group, e *GroupUser) { n.Edges.GroupUsers = append(n.Edges.GroupUsers, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (gq *GroupQuery) loadService(ctx context.Context, query *ServiceQuery, nodes []*Group, init func(*Group), assign func(*Group, *Service)) error {
...
func (gq *GroupQuery) loadGroupUsers(ctx context.Context, query *GroupUserQuery, nodes []*Group, init func(*Group), assign func(*Group, *GroupUser)) error {
...
func (gq *GroupQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := gq.ctx.Fields; len(fields) > 0 {
		...
		if gq.withService != nil {
			_spec.Node.AddColumnOnce(group.FieldServiceID)
		}
	}
