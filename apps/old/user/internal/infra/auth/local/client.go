...
func NewGRPCClient(addr string) (localuserv1.LocalUserServiceClient, *grpc.ClientConn, error) {
	...
	if healthResp.Status != grpc_health_v1.HealthCheckResponse_SERVING {
		return nil, nil, errors.New("local user service is not serving")
	}

	client := localuserv1.NewLocalUserServiceClient(conn)
	if client == nil {
